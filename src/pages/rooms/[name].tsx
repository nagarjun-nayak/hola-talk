    ...
    <>
      {data && (
        <LiveKitRoom
          ...
          options={roomOptions}
          video={userChoices.videoEnabled}
          audio={true} // Audio is enabled at the room level
          onDisconnected={onLeave}
        >
          {/* This new component is now correctly placed inside LiveKitRoom */}
          <RoomUI
            roomName={roomName}
            userId={userId}
...
// --- NEW COMPONENT THAT FIXES THE ERROR ---
type RoomUIProps = {
  roomName: string;
  userId: string;
  selectedLanguage: string;
...
const RoomUI = ({ roomName, userId, selectedLanguage, initialAudioEnabled }: RoomUIProps) => {
  // These hooks now work correctly because they are inside the LiveKitRoom context
  const { localParticipant } = useLocalParticipant();
  const isMicrophoneEnabled = localParticipant.isMicrophoneEnabled;
  ...
  // This effect sets the initial microphone state from your PreJoin choice
  useEffect(() => {
    if (connectionState === 'connected' && localParticipant.isMicrophoneEnabled !== initialAudioEnabled) {
      localParticipant.setMicrophoneEnabled(initialAudioEnabled);
    }
  }, [connectionState, localParticipant, initialAudioEnabled]);

  // This effect handles receiving transcriptions from other users
  useEffect(() => {
    if (!process.env.NEXT_PUBLIC_PUSHER_KEY || !process.env.NEXT_PUBLIC_PUSHER_CLUSTER) {
      console.error("Pusher environment variables not set.");
      return;
